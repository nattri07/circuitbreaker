...
def postreq(*args, **kwargs):
	...
	createHashEntry(*args)								#doesnt exist in circuit
	...
	if int(redisDB.hget("circuitStatus", args)) == 1:	#check if circuit is live
		...
		isLive = updateStatus(code,*args)				#update service metrics and issue trip
		if not isLive:
			print ""
			restore(*args)
...
def updateStatus(result,*args):
	...
	if successRate > TOLERANCE_THRESHOLD:				#WOOHOO still not failing
		...
		return trip(*args)								#shucks trip me
